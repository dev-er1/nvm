// nvm-core/src/vm/jumptable.rs
//
//! # Jump Table исполнитель
//!
//! В этом модуле реализован исполнитель инструкций на основе
//! *jump table* (таблицы переходов).
//!
//! ## Идея
//!
...
//!
//! Слот `0` — заголовок: **индекс диспетчеризации** = `опкод * 8 + виды
//! операндов`, где "виды" — по одному биту на операнд ("операнд —
//! регистр"). Таблица переходов имеет размер `опкоды * 8` и по одному
//! **специализированному** обработчику на каждую валидную сигнатуру
//! (например, `IADD` — четыре: `(imm, imm)`, `(reg, imm)`,
//! `(imm, reg)`, `(reg, reg)`). В обработчике не остаётся веток
//! по виду операнда — она устранена уже при диспетчеризации.
//!
...
//! ## Горячий цикл
//!
//! В цикле на инструкцию приходится: чтение заголовка без проверки
//! границ (инвариант `ip < len`), косвенный вызов обработчика и одна
//! проверка следующего `ip` на выход за конец программы. Операнды
//! читаются обработчиком по сырому указателю без копий.
//!
...
/// `0 <= ip < instruction_count` в [`NVM::jumptable_execute`]);
...
macro_rules! call_variant {
    ($name:ident, $k:tt) => {
        fn $name(vm: &mut NVM, p: *const u64, ip: usize) -> HandlerResult {
            ...
            // Запоминаем адрес возврата. Семантика совпадает
            // с `default::match_execute`.
            vm.call_stack.push(ip);
            Ok(target)
        }
    };
}
...
///
/// **Индекс вычисляется при кодировании программы** ([`encode`]),
/// поэтому в горячем цикле достаточно одного обращения к таблице —
/// без маскирования и сдвигов.
static JUMP_TABLE: [Handler; TABLE_LEN] = build_jump_table();

// ====== Исполнитель ======

impl NVM {
    /// Выполняет программу на основе jump table.
    pub fn jumptable_execute(&mut self) -> Result<(), VMError> {
        ...
        loop {
            ...
            let handler = JUMP_TABLE[unsafe { *base } as usize];
...
/// `[заголовок, операнд1, операнд2, операнд3]`. В заголовке —
/// индекс диспетчеризации `опкод * 8 + виды операндов`, где виды —
/// по биту на операнд (`1` — регистр).
///
...
fn encode(program: &[Instruction]) -> Result<Vec<u64>, VMError> {
    ...
    for instr in program {
        ...
        code.push(instr.opcode as u64 * 8 + kinds);
